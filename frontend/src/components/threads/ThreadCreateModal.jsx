...
export default function ThreadCreateModal({
  ...
  const submit = async (e) => {
    ...
    // FIXED: Properly use username/fullName
    const displayName =
    ...
    await onSubmit?.({
      ...
      tags,
      author: currentUser
        ? { id: currentUser.id, name: displayName }
        : { id: 0, name: "Unknown" },
    });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div
        className="bg-white w-full max-w-2xl rounded-xl border border-[#e3d8c8] shadow-lg overflow-hidden"
        role="dialog"
        aria-modal="true"
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 rounded border border-[#ddcdb7] bg-white hover:bg-[#f7ecda] text-sm"
              style={{ fontFamily: "Times New Roman, serif" }}
            >
              Cancel
            </button>
            <button
              ...
              className="px-4 py-2 rounded border border-[#ddcdb7] bg-[#efe6d7] hover:bg-[#e3d5c2] text-sm disabled:opacity-60"
              style={{ fontFamily: "Times New Roman, serif" }}
            >
              Create
            </button>
